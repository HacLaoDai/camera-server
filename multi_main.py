...
# -------------------------------------------------------------------------
# HEADLESS + MJPEG STREAM
#
# Container chạy trên server không có màn hình -> KHÔNG dùng cv2.imshow
# (sẽ crash vì không tìm được display). Thay vào đó, mỗi vòng lặp build
# lưới ảnh như cũ, encode sang JPEG rồi giữ trong bộ nhớ (RAM) để 1 HTTP
# server nhỏ (stdlib, không thêm dependency) phát ra dạng
# "multipart/x-mixed-replace" (MJPEG) - máy khác chỉ cần mở trình duyệt
# trỏ tới http://<ip>:<STREAM_PORT>/stream (xem cả lưới) hoặc
# http://<ip>:<STREAM_PORT>/stream/<channel> (xem riêng 1 camera) là xem
# được video trực tiếp, không cần cài thêm phần mềm gì.
STREAM_HOST = os.environ.get("STREAM_HOST", "0.0.0.0")
STREAM_PORT = int(os.environ.get("STREAM_PORT", "8090"))
STREAM_FPS = float(os.environ.get("STREAM_FPS", "10"))  # fps phát ra cho client xem, không phải fps xử lý
JPEG_QUALITY = int(os.environ.get("STREAM_JPEG_QUALITY", "70"))

# Vòng lặp chính (build_grid) chạy nhanh cỡ nào thì không cần đụng tới
# STREAM_FPS - đây chỉ giới hạn tốc độ ENCODE+GHI RAM để không lãng phí CPU
# khi build lưới quá nhanh so với nhu cầu xem.
MAIN_LOOP_SLEEP = 1.0 / max(STREAM_FPS, 1.0)
...
# -------------------------------------------------------------------------
# Trạng thái dùng chung giữa vòng lặp chính (ghi frame) và HTTP server (đọc
# frame để phát MJPEG) - module-level vì BaseHTTPRequestHandler được
# ThreadingHTTPServer khởi tạo mới cho MỖI request, không có cách nào
# truyền tham số vào __init__ theo cách thông thường.
_stream_state = {
    "output_frames": None,   # dict {cam_name: latest_bgr_frame} - set trong main()
    "cam_names": None,       # list thứ tự camera hiển thị trong grid
    "stop_event": None,
}
...
class MJPEGRequestHandler(BaseHTTPRequestHandler):
    # Tắt log mặc định (in ra mỗi request) - đã có logging riêng qua logger,
    # tránh làm loãng log file khi có client xem stream liên tục.
    def log_message(self, format, *args):
        pass

    def _write_jpeg_stream(self, frame_getter):
        ...
        stop_event = _stream_state["stop_event"]
        ...
        try:
            ...
            # Client đóng tab / mất kết nối - bình thường, không phải lỗi.
            pass
    ...
    def do_GET(self):
        ...
        if path == "/stream":
            def getter():
                output_frames = _stream_state["output_frames"]
                cam_names = _stream_state["cam_names"] or []
                if output_frames is None:
                    return None
                return build_grid(output_frames, cam_names)

            self._write_jpeg_stream(getter)
            return

        if path.startswith("/stream/"):
            cam_name = path[len("/stream/"):]

            def getter():
                output_frames = _stream_state["output_frames"]
                if output_frames is None:
                    return None
                return output_frames.get(cam_name)
...
def start_mjpeg_server(output_frames, cam_names, stop_event):
    """Khởi động HTTP server MJPEG trong 1 thread nền daemon. Trả về server
    object để có thể shutdown() gọn gàng khi chương trình dừng."""
    _stream_state["output_frames"] = output_frames
    _stream_state["cam_names"] = cam_names
    _stream_state["stop_event"] = stop_event

    server = ThreadingHTTPServer((STREAM_HOST, STREAM_PORT), MJPEGRequestHandler)
    server.daemon_threads = True

    thread = threading.Thread(target=server.serve_forever, daemon=True, name="MJPEGServer")
    thread.start()

    logger.info(
        f"[MJPEG] Stream server sẵn sàng tại http://{STREAM_HOST}:{STREAM_PORT}/stream "
        f"(toàn bộ lưới) hoặc /stream/<channel> (từng camera riêng)"
    )
...
def main():
    ...
    # HEADLESS: không mở cửa sổ GUI (container không có display). Thay vào
    # đó chạy 1 HTTP server MJPEG để máy khác xem video qua trình duyệt -
    # xem chi tiết ở start_mjpeg_server()/MJPEGRequestHandler phía trên.
    mjpeg_server = start_mjpeg_server(output_frames, cam_names, stop_event)

    # Dừng gọn gàng khi `docker stop` gửi SIGTERM (mặc định 10s rồi mới bị
    # kill -9) hoặc Ctrl+C (SIGINT) lúc chạy thử ngoài Docker - thay cho
    # phím "q" của bản GUI cũ.
    def _handle_shutdown_signal(signum, frame):
        logger.info(f"Nhận tín hiệu dừng ({signum}) - đang tắt gọn gàng...")
        stop_event.set()
    ...
    logger.info("Pipeline đang chạy ở chế độ headless (không GUI).")

    try:
        while not stop_event.is_set():
            # Không cần tự build_grid() ở đây nữa - MJPEGRequestHandler tự
            # build_grid() theo đúng lúc có client đang xem, tránh lãng phí
            # CPU encode JPEG liên tục khi không ai xem stream.
        ...
        logger.info("Đang dừng các camera pipeline...")
        for w in workers:
            w.join(timeout=5)
